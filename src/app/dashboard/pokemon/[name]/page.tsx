import { Pokemon, PokemonResponse, Result } from "@/types/pokemons";
...
interface Props {
  params: {
    name: string; // all params are strings
  };
}
...
export async function generateStaticParams () {
  ...
  const static151Names = data.results.map((pokemon: Result) => ({
    name: pokemon.name,
  }));


  return static151Names.map((pokemon) => ({ name: pokemon.name }));
};
